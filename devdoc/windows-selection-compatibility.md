...
鼠标拖选和双击选词工具条与本页历史快捷键测试分开：工具条在鼠标松开后只用 UI Automation 被动读取选区，不触碰剪贴板；读取不到文字时不显示。`Alt+X` 快捷键仍保留 UIA 及剪贴板回退。下方历史记录不代表工具条已在这些应用中通过人工验收。

当前 `Alt+X` 行为（2026-09-25）：读取到文字时弹出 Popup 并翻译；确认选区为空时弹出可手动输入的空白 Popup，不调用翻译 Provider；读取失败仍显示错误 Popup。下方较早的“无选区不弹窗”记录保留为当时的测试结果。
...
## 2026-09-25 选词工具条本机验证

Windows Release 构建在记事本和隔离配置的 Chrome 本地测试页中，用真实鼠标拖选均显示两按钮工具条；Chrome 页面空白处点击会关闭它。记事本选择 `Hello Lexift too` 后，复制写入相同文字，工具条保留且前台窗口仍是记事本。点击翻译后 Popup 显示相同原文；DeepL 返回 `rate limit exceeded`，因此本次未验证成功译文。Edge 独立测试尚未确认工具条表现，不以 Chrome 的结果代替 Edge。

双击选词随后加入同一工具条读取流程；上段人工验证只覆盖拖选，不代表双击已完成人工验收。
...
5. 从跟踪记录中记录 `strategy`、`uia_latency_ms`、`clipboard_latency_ms` 和 `total_capture_ms`。
   切勿将源文本复制到此文档或日志中。
6. 在没有选择的情况下，以及快速按两次 `Alt+X` 的情况下重复测试。无选择应进入 NoSelection 并弹出空白 Popup，不自动翻译；进行中的捕获必须忽略重复请求。
