...
!define NOTIFYICONSETTINGSKEY "Control Panel\NotifyIconSettings"
!define DEEPLCREDENTIALTARGET "Lexift/deepl-primary"
!define LEXIFT_ERROR_NOT_FOUND 1168
...
; Remove only the Windows tray history entry owned by this installed executable.
; Entries for development builds or another Lexift installation path are preserved.
Function un.RemoveInstalledTrayHistory
  StrCpy $0 0
  ${UnStrCase} $R8 "$INSTDIR\${MAINBINARYNAME}.exe" "L"

  tray_history_loop:
    EnumRegKey $1 HKCU "${NOTIFYICONSETTINGSKEY}" $0
    StrCmp $1 "" tray_history_done
    ReadRegStr $2 HKCU "${NOTIFYICONSETTINGSKEY}\$1" "ExecutablePath"
    ${UnStrCase} $R9 $2 "L"
    StrCmp $R9 $R8 tray_history_match tray_history_next

  tray_history_match:
    ClearErrors
    DeleteRegKey HKCU "${NOTIFYICONSETTINGSKEY}\$1"
    IfErrors tray_history_error
    DetailPrint "Removed Lexift tray history for $2"
    ; Deleting the current key shifts the next key into the same enumeration index.
    Goto tray_history_loop

  tray_history_error:
    DetailPrint "Could not remove Lexift tray history for $2"
...
; Delete the V0.1 DeepL credential only when the user explicitly requests removal
; of application data. A missing credential is already the desired result.
Function un.DeleteStoredCredentials
  ClearErrors
  System::Call 'Advapi32::CredDeleteW(w "${DEEPLCREDENTIALTARGET}", i 1, i 0) i.r0 ?e'
  Pop $1
  StrCmp $0 0 credential_delete_failed credential_delete_done

  credential_delete_failed:
    StrCmp $1 ${LEXIFT_ERROR_NOT_FOUND} credential_delete_done
    DetailPrint "Could not delete Windows credential ${DEEPLCREDENTIALTARGET} (error $1)"
    StrCpy $UninstallCleanupFailed 1
    Goto credential_delete_done

  credential_delete_done:
FunctionEnd

; Remove registry state owned by this installer. This function is intentionally
; idempotent so un.onUninstSuccess can repeat the cleanup after all sections finish.
Function un.RemoveInstallerRegistry
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "Lexift"
...
Section Uninstall
  ...
  ; Remove only Shell history that belongs to this installed executable.
  Call un.RemoveInstalledTrayHistory
  ...
  ${If} $DeleteAppDataCheckboxState == 1
      ...
      SetShellVarContext current
      Call un.DeleteStoredCredentials
      {{#each appdata_paths}}
      RmDir /r "{{unescape_dollar_sign this}}"
      IfFileExists "{{unescape_dollar_sign this}}" 0 +3
        DetailPrint "Could not completely remove {{unescape_dollar_sign this}}"
        StrCpy $UninstallCleanupFailed 1
      {{/each}}
  ${Else}
      DetailPrint "Delete user data not selected; preserving settings, logs and credentials"
  ${EndIf}
  {{/if}}
  ...
  ${If} $UninstallCleanupFailed == 1
    DetailPrint "Lexift uninstall completed with cleanup errors"
    SetErrors
    SetErrorLevel 1
  ${EndIf}
SectionEnd
...
Function un.onUninstSuccess
  ; Perform a final pass after every uninstall section and MUI callback has completed.
  Call un.RemoveInstallerRegistry
FunctionEnd
...
Function RestorePreviousInstallLocation
  ...
  restore_location_use:
    StrCpy $INSTDIR $4
    Goto restore_location_done

  restore_location_stale:
    ; Ignore and remove invalid history instead of redirecting a fresh install to it.
    DeleteRegKey SHCTX "${MANUPRODUCTKEY}"
    DeleteRegKey /ifempty SHCTX "Software\${MANUFACTURER}"

  restore_location_done:
FunctionEnd
